...
function points(n){score+=n;if(score>best){best=score;save();}}
...
function addEntity(type,x,z,options={}){
 const person=type==='person',mesh=person?PolarityWorld.character(scene,options.model||0):PolarityWorld.object(scene,type,options.color),e={id:'world-'+entities.length,type,name:options.name||({crate:'Metalowa skrzynia',cell:'Ogniwo energii',beam:'Stalowa belka',bench:'Ławka',barrel:'Metalowa beczka'}[type]),person,mesh,x,y:0,z,vx:0,vy:0,vz:0,startX:x,startZ:z,radius:person?.38:type==='beam'?1.7:type==='bench'?1.1:.55,height:person?1.85:type==='beam'?.7:1.1,health:75,knocked:0,scared:0,hit:0,thrown:0,spin:0,angle:Math.PI,worker:!!options.worker,removed:false,phase:Math.random()*6};mesh.position.set(x,0,z);entities.push(e);return e;
}
...
function resetEntity(e){e.x=e.startX;e.z=e.startZ;e.y=0;e.vx=e.vy=e.vz=0;e.health=75;e.knocked=0;e.thrown=0;e.spin=0;e.respawnTime=0;e.retired=false;e.scared=0;e.hit=0;e.mesh.rotation.set(0,Math.PI,0);if(held===e)held=null;}
...
function updateEntities(dt){
 for(const e of entities){if(e.respawnTime>0){e.respawnTime=Math.max(0,e.respawnTime-dt);if(e.respawnTime===0)respawnPerson(e);continue;}if(e.removed)continue;e.scared=Math.max(0,e.scared-dt);e.hit=Math.max(0,e.hit-dt);e.thrown=Math.max(0,e.thrown-dt);
  if(e!==held){
  ...
  if(e.person){if(e.knocked>0){e.knocked-=dt;if(e.knocked<=0&&e!==held){if(!e.rescue)e.health=75;e.mesh.rotation.z=0;}}const state=e.knocked>0?(e.health===0?'Death':'HitRecieve'):e===held?'Wave':e.rescue&&!e.retired?(e.moving?'Run':'Idle'):e.worker?'Idle':e.scared?'Run':'Walk';PolarityWorld.animate(e.mesh,state,dt);}
...
function screen(name){if(mode==='play')saveProgress();mode=name;releaseInputs();needsRender=true;for(const id of ['menu','pause','settings'])$(id).hidden=id!==name;$('hud').hidden=name!=='play';if(name!=='play'){stopKnockoutSound();if(document.pointerLockElement)document.exitPointerLock();$(name).querySelector('button')?.focus({preventScroll:true});}}
...
$('game').addEventListener('pointerdown',event=>{if(mode!=='play')return;if(event.pointerType==='mouse'&&document.pointerLockElement===$('game')&&event.button===0){keys.add('Mouse0');action('use');}else{lookPointer={id:event.pointerId,x:event.clientX,y:event.clientY,moved:false,button:event.button};$('game').setPointerCapture(event.pointerId);}});
...
let last=0,hudTime=0,saveTime=0;function frame(now){const dt=Math.min((now-last)/1000||0,.08);last=now;if(mode==='play'){time+=dt;updatePlayer(dt);updateEntities(dt);updateMissions();saveTime+=dt;if(saveTime>=1){saveProgress();saveTime=0;}toastTime-=dt;if(toastTime<=0)$('toast').style.opacity=0;for(let i=effects.length-1;i>=0;i--){const e=effects[i];e.life-=dt;e.mesh.material.opacity=Math.max(0,e.life/e.total);if(e.life<=0){scene.remove(e.mesh);e.mesh.geometry.dispose();e.mesh.material.dispose();effects.splice(i,1);}}hudTime+=dt;if(hudTime>.08){updateUI();hudTime=0;}}
...
soundLabel();
(async()=>{try{renderer=new THREE.WebGLRenderer({canvas:$('game'),antialias:true,powerPreference:'high-performance'});renderer.setPixelRatio(Math.min(devicePixelRatio,1.5));renderer.outputColorSpace=THREE.SRGBColorSpace;renderer.toneMapping=THREE.ACESFilmicToneMapping;renderer.toneMappingExposure=1;renderer.shadowMap.enabled=true;renderer.shadowMap.type=THREE.PCFSoftShadowMap;scene=new THREE.Scene();camera=new THREE.PerspectiveCamera(65,innerWidth/innerHeight,.05,240);camera.position.set(player.x,1.66,player.z);camera.rotation.x=player.pitch;await Promise.all([PolarityWorld.load(),prepareKnockoutSound()]);build();const continued=loadProgress();resize();window.addEventListener('resize',resize);ready=true;$('start').disabled=false;$('start').textContent=continued?'KONTYNUUJ GRĘ ↗':'URUCHOM SWOJE MOCE ↗';updateEntities(0);updatePlayer(0);updateUI();requestAnimationFrame(frame);$('game').addEventListener('webglcontextlost',event=>{event.preventDefault();ready=false;screen('menu');$('start').disabled=true;$('loadError').hidden=false;});}catch(error){console.error(error);$('loadError').hidden=false;$('start').disabled=true;}})();
