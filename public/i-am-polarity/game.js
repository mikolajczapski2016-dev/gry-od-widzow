...
function action(name){if(mode!=='play')return;if(curse){if(name==='use'||name==='throw')curseStrike();return;}if(name==='fly')toggleFlight();if(name==='grab')grab();if(name==='throw')toss();if(name==='switch')switchTool();if(name==='jump')jump();if(name==='use')tool==='pistol'?shoot():toss();}
...
function rotate(dx,dy){const viewer=curse?curse.hero:player;viewer.yaw+=dx*.0025*sensitivity;viewer.pitch=clamp(viewer.pitch-dy*.0025*sensitivity,-1.1,1.1);needsRender=true;}
...
let last=0,hudTime=0,saveTime=0;function frame(now){const dt=Math.min((now-last)/1000||0,.08);last=now;if(mode==='play'){time+=dt;if(curse)updateCurse(dt);else updatePlayer(dt);updateEntities(dt);updateMissions();saveTime+=dt;if(saveTime>=1){saveProgress();saveTime=0;}toastTime-=dt;if(toastTime<=0)$('toast').style.opacity=0;for(let i=effects.length-1;i>=0;i--){const e=effects[i];e.life-=dt;e.mesh.material.opacity=Math.max(0,e.life/e.total);if(e.life<=0){scene.remove(e.mesh);e.mesh.geometry.dispose();e.mesh.material.dispose();effects.splice(i,1);}}hudTime+=dt;if(hudTime>.08){updateUI();hudTime=0;}}
