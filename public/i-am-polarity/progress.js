...
const progressBackupKey=progressKey+'-backup';
let progressWriteBlocked=false,lastGoodProgress=null;
...
function loadProgress(recoveryRaw=null){
 try{
  ...
  // Never let a fresh session overwrite progress that an update could not read.
  progressWriteBlocked=true;
  if(!recoveryRaw){try{const backup=localStorage.getItem(progressBackupKey);if(backup)return loadProgress(backup);}catch{}}
  $('saveStatus').textContent='Nie udało się wczytać postępów. Twój zapis został zachowany; automatyczny zapis jest wstrzymany, aby go nie nadpisać.';return false;
 }
